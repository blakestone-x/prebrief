...
from ._identity import project_from_path as _canonical_project_from_path
...
def project_from_path(path, explicit=None):
    """Resolve tenant identity through the shared pure implementation."""
    def marker_reader(directory):
        marker = os.path.join(directory, ".prebrief-project")
        if not os.path.isfile(marker):
            return None
        with open(marker, encoding="utf-8") as fh:
            return (fh.readline() or "").strip()

    return _canonical_project_from_path(
        path,
        explicit=explicit,
        env=os.environ,
